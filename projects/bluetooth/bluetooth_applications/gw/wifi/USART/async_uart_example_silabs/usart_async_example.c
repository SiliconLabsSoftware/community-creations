  ...
  switch (current_mode) {
    ...
    case SL_USART_SEND_DATA:
      ...
      if (usart_begin_transmission ) {
        ...
        // Nap vao buffer xong thi goi API send
    ...
    case SL_USART_RECEIVE_DATA:
      if (usart_begin_transmission == true) {
        ...
        status = sl_si91x_usart_receive_data
            (usart_handle, usart_data_in, sizeof(usart_data_in));
        ...
        for (size_t i = 0; i < sizeof(usart_data_in); i++) {
            DEBUGOUT("%02X ", usart_data_in[i]);
        }
        DEBUGOUT("\n");

        // Dat flag tranh goi API nhieu lan
        usart_begin_transmission = false;
      }
